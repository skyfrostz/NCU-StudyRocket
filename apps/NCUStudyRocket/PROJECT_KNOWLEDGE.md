...
初次知识整理清点了 29 个早期会话。2026-09-24 又以只读 Codex 索引复查 2026-08-25 至 2026-09-24 的全部项目记录：26 个有实质消息的主对话、3 个空白或仅环境消息的主记录、47 个支援子对话，另有 1 条索引记录缺少日志文件。全量记录用于覆盖统计；涉及运行、写入、部署和学业事实的主对话逐条核对。详见 `CONVERSATION_AUDIT_2026-09-24.md`。
...
### 工具可见但 `propose_changes` 返回 unsupported

1. 先确认回合从 StudyRocket Desktop 或 Host 聊天入口发起。Codex 历史页直接续聊没有 StudyRocket 的工具处理器，即使任务仍保存动态工具声明也会失败。
2. `dynamicToolsReady=true` 只证明 Host 当前协议门禁打开；必须再发起一个真实回合并观察 SSE/历史终态。
3. 读取 `/v1/proposals`，核对目标路径、`baseHash`、候选正文和管理标记。只有 proposal store 出现正确草案才算提交成功。
4. `thread not found`、`active writer`、401、429 和超时是不同故障，按 `$studyrocket-maintenance` 的错误分类处理；不要统一升级协议或重复重发。
...
- `6ad2bd2` 修复历史读取成功后聊天状态仍停在 unavailable，并在 Desktop 明确提示 Codex 页面只用于查看历史。
- `dbe0711` 处理 Desktop 已占有固定任务时的上下文保留迁移，以及精确 `thread not found` 的一次性恢复重试；两个提交均已推送 `origin/main`。
- 真实 Host 已完成一个 StudyRocket 发起的回合，并生成 `工作台/下周计划.md` 草案；草案以 `779b393d8cb6c274deea49012090f05b248539962c49faf5c376e257f3b67488` 为 `baseHash`，管理标记不变。用户随后已在应用中确认；2026-09-24 11:00 复核时 proposal store 为空，计划中已包含 9 月 23—27 日目标调整，当前文件哈希为 `7e21c45bc89ce8e03cc418997c0290691cbe905f3a994eb013492ac40e54cdcc`。
- 中间出现的 `429 Too Many Requests` 属远端限流，与动态工具路由修复分开；后续一次重试成功，不将 429 当作协议回归。
