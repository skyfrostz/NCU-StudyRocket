...
struct NCUStudyRocketApp: App {
    ...
    var body: some Scene {
        WindowGroup("NCU StudyRocket") { ContentView().environmentObject(workspace).environmentObject(document).environmentObject(chat).environmentObject(reminders).frame(minWidth: 980, minHeight: 680) }
