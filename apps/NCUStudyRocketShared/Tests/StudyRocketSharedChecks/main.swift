...
// A recovered local connection must reopen chat without asserting that model
// authentication succeeded or clearing an already observed auth failure.
precondition(!StudyRocketChatState.unavailable.canGenerate)
precondition(StudyRocketChatState.unavailable.afterProtocolReconnect.canGenerate)
precondition(StudyRocketChatState.starting.afterProtocolReconnect == .protocolReadyAuthUnknown)
precondition(StudyRocketChatState.ready.afterProtocolReconnect == .ready)
precondition(StudyRocketChatState.authFailed.afterProtocolReconnect == .authFailed)
precondition(!StudyRocketChatState.authFailed.afterProtocolReconnect.canGenerate)
